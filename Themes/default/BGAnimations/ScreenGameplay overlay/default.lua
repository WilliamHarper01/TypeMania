							  ...
							  SetCommand=function(self)
								...
								if song then
									if steps_data ~= nil then
										...
										local diff = steps_data:GetDifficulty();
										local cd = GetCustomDifficulty(st, diff);
										self:settext(steps_data:GetMeter()):diffuse(color("#ffffff")):diffusealpha(0.8);
									else
										self:settext("")
									end
								else
			  ...
			  SetCommand=function(self)
				...
				if song then
					if stepsP1 ~= nil then
						...
						local diff = stepsP1:GetDifficulty();
						local cd = GetCustomDifficulty(st, diff);
						self:settext(stepsP1:GetMeter()):diffuse(color("#ffffff")):diffusealpha(0.8);
					else
						self:settext("")
					end
				else
			  ...
			  SetCommand=function(self)
				...
				if song then
					if stepsP2 ~= nil then
						...
						local diff = stepsP2:GetDifficulty();
						local cd = GetCustomDifficulty(st, diff);
						self:settext(stepsP2:GetMeter()):diffuse(color("#ffffff")):diffusealpha(0.8);					
						else
						self:settext("")
					end
				else
